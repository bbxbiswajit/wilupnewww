...
export const LandingView: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (USE_MOCK_BACKEND) {
      const handleMockAudit = (e: any) => {
        const payload = e.detail;
        if (payload.new) {
          setAuditLogs((prev) => [...prev, payload.new]);
        }
      };
      window.addEventListener(`audit-logs-ticket-${ticket.id}`, handleMockAudit);
      return () => {
        window.removeEventListener(`audit-logs-ticket-${ticket.id}`, handleMockAudit);
      };
    }
