...
export function useTicketRealtime({
  ...
  useEffect(() => {
    ...
    if (USE_MOCK_BACKEND) {
      ...
      const eventName = ticketId ? `ticket-update-${ticketId}` : 'mock-cdc-event';
      window.addEventListener(eventName, handleMockEvent);
      
      return () => {
        isMounted = false;
        window.removeEventListener(eventName, handleMockEvent);
      };
    }
